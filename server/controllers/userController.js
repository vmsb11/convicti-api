        ...
        try {
            ...
            if(userCollection) {
                ...
                if(userCollection.status === 'Ativo') {
                    ...
                    const { userId, occupation } = userCollection;
                    //gera o token de autorização
                    const token = jwt.sign({ userId }, process.env.SECRET, {
                        expiresIn: 300000
                    });

                    //verifica qual a ocupação do usuário e faz o vínculo do usuário com o respectivo cargo
                    switch(occupation) {

                        case 'Diretor Geral':

                            //como o usuário é um diretor, recupera o id do diretor
                            const generalDirector = await DirectorPersistence.findDirectorByParameters([
                                { "field": "userId", "value": userId }
                            ]);

                            //envia como resposta o usuário autenticado
                            res.status(200).send({ auth: true, token: token, user: userCollection, director: generalDirector });
                        break;
                        case 'Diretor':

                            //como o usuário é um diretor, recupera o id do diretor
                            const director = await DirectorPersistence.findDirectorByParameters([
                                { "field": "userId", "value": userId }
                            ]);

                            //envia como resposta o usuário autenticado
                            res.status(200).send({ auth: true, token: token, user: userCollection, director: director });
                        break;
                        case 'Gerente':

                            //como o usuário é um gerente, recupera o id do gerente
                            let manager = await ManagerPersistence.findManagerByParameters([
                                { "field": "userId", "value": userId }
                            ]);

                            //envia como resposta o usuário autenticado
                            res.status(200).send({ auth: true, token: token, user: userCollection, manager: manager });
                        break;
                        case 'Vendedor':

                            //como o usuário é um vendedor, recupera o id do vendedor
                            let seller = await SellerPersistence.findSellerByParameters([
                                { "field": "userId", "value": userId }
                            ]);

                            //envia como resposta o usuário autenticado
                            res.status(200).send({ auth: true, token: token, user: userCollection, seller: seller });
                        break;
                    }
                }
                else {
